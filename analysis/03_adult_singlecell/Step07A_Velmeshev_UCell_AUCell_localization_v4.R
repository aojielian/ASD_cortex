...
DATASET_TAG <- "Velmeshev"
PREFIX <- "95_Velmeshev"
LOGFILE <- file.path(LOGDIR, "Step07_Velmeshev_UCell_AUCell_localization_v4.log")
log_msg <- function(...) {
...
if (is.na(args$batch_size)) args$batch_size <- 8000L
BATCH_SIZE <- as.integer(args$batch_size)
NCORES_USE <- max(1L, min(4L, as.integer(Sys.getenv("SLURM_CPUS_PER_TASK", "1"))))

DATA_BASE <- "/gpfs/hpc/home/lijc/lianaoj/autism_scRNA/Velmeshev_scRNA"
META_FILE <- file.path(DATA_BASE, "meta.tsv")
RAW_ZIP   <- file.path(DATA_BASE, "rawMatrix.zip")
PROGRAM_FILE <- file.path(TABDIR, "20_midPrenatal_core_programs.tsv")

map_velmeshev_broad_class <- function(cluster) {
  cluster <- as.character(cluster)
  out <- rep(NA_character_, length(cluster))
  out[grepl("^IN-", cluster)] <- "INN"
  out[grepl("^L|^Neu-", cluster)] <- "EXN"
  out[cluster %in% c("AST-FB", "AST-PP")] <- "AST"
  out[cluster == "OPC"] <- "OPC"
  out[cluster == "Oligodendrocytes"] <- "ODC"
  out[cluster == "Microglia"] <- "MG"
  out[cluster == "Endothelial"] <- "END"
  out
}
...
req <- c("cell", "cluster", "sample", "individual", "region", "age", "sex", "diagnosis")
if (!all(req %in% names(meta))) stop("Velmeshev meta missing required columns.")
meta[, cell := as.character(cell)]
meta[, subtype := as.character(cluster)]
meta[, broad_class := map_velmeshev_broad_class(cluster)]
meta[, individual := as.character(individual)]
meta[, diagnosis := factor(as.character(diagnosis), levels = c("Control", "ASD"))]
...
if (!file.exists(RAW_ZIP)) stop("Missing rawMatrix.zip: ", RAW_ZIP)
tmpdir <- tempfile(pattern = "vel_raw_")
dir.create(tmpdir, recursive = TRUE, showWarnings = FALSE)
unzip(RAW_ZIP, files = c("genes.tsv", "barcodes.tsv", "matrix.mtx"), exdir = tmpdir)

gdt <- fread(file.path(tmpdir, "genes.tsv"), header = FALSE, sep = "\t")
symbols <- if (ncol(gdt) >= 2) gdt[[2]] else gdt[[1]]
barcodes <- fread(file.path(tmpdir, "barcodes.tsv"), header = FALSE)$V1

log_msg("Reading Velmeshev sparse matrix: ", file.path(tmpdir, "matrix.mtx"))
mat <- readMM(file.path(tmpdir, "matrix.mtx"))
mat <- as(mat, "CsparseMatrix")
colnames(mat) <- as.character(barcodes)

log_msg("Deduplicating Velmeshev gene symbols by max-row-sum representative")
mat <- dedup_keep_max(mat, symbols)

common_cells <- intersect(colnames(mat), meta_keep$cell)
log_msg("Velmeshev matched cells: ", length(common_cells))
